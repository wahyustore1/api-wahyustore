...
function generateTransactionId() {
    return `WahyuSTRID-${crypto.randomBytes(2).toString('hex').toUpperCase()}`
}
...
let { directLink } = await service.uploadFromBinary(buffer, 'wahyustr.png');
            resolve(directLink);
        } catch (error) {
...
async function generateQRIS(amount) {
    try {
        let qrisData = "00020101021126670016COM.NOBUBANK.WWW01189360050300000879140214149145236654400303UMI51440014ID.CO.QRIS.WWW0215ID20253890762610303UMI5204541153033605802ID5921WAHYU STORE OK23497026012GUNUNG KIDUL61055580062070703A0163044648";
...
async function checkQRISStatus() {
    try {
        const apiUrl = `https://gateway.okeconnect.com/api/mutasi/qris/OK2349702/615013617429556662349702OKCT50675AD0EB459FC5509E607E9F6FC97D`;
